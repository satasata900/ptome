                        ...
                        $member_nick_name = $details['member_nick_name'];
                            ...
                            if ($user) {
                                if ($user['id'] == $user_id) {
                                    ...
                                    $sth= $con->prepare("SELECT a.*, b.user_name FROM members_book a INNER JOIN users b ON a.member_id = b.id WHERE a.user_id=? AND a.member_id=?");
                                    $sth->execute(array($user_id,$user['id']));
                                    $member_found = $sth->fetch(\PDO::FETCH_ASSOC);
                                    if ($member_found) {
                                        $response['message'] = $member_found['user_name'] . ' ' . translate('member_book_present');
                                        $response['success'] = false;  
                                    } else {
                                        $data = array(
                                            'user_id'=>$user_id,
                                        ...
                                        $insert_request = $con->prepare(insert('members_book', $data))->execute();
                                        if ($insert_request) {
