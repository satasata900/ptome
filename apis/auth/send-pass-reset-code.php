        ...
        if (empty($details['email'])) {
            ...
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                if ($user) {
                    ...
                    $stmt=$con->prepare("UPDATE users SET pass_reset_code=? WHERE id=? ");
                    $stmt->execute(array(sha1($ran_no),$user['id']));
                    $response['success'] = true;
                    $response['message'] = translate('pass_reset_code_sent');
                    /****************************************************************************************/
                    ...
                    $message .= '<p>We cannot simply send you your old password. A unique code to reset your password has been generated for you.</p>';
                    $message .= '<p>Password reset code is : <span class="code" style="font-size: 1.4rem;font-weight: 700;color: #387dec;">' . $ran_no . '</span></p>';
                    $message .= '<p>Enter this code in our application to reset your password.</p>';
