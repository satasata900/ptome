            ...
            if (getUserActiveStatus($con, $user_id)) {
                ...
                if (empty($details['service_id'])) {
                    ...
                    if ($service) {
                        ...
                        if ($member) {
                            $response['message'] = translate('already_subscribed');
                            $response['success'] = false;
                        } else {
                            $service_data = array(
                                'service_id'=>$service_id,
                                'member_id'=>$user_id,
                                'subscribtion_date'=>time()
                            );
                            $insert_request = $con->prepare(insert('service_members', $service_data))->execute();
                            if ($insert_request) {
                                $response['message'] = translate('successfully_subscribed');
